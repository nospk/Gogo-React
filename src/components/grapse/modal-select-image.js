...
export const CALLBACK_MODAL_SELECT_IMAGE = "CALLBACK_MODAL_SELECT_IMAGE"
export const ModalSelectImage = (editor)=>{
    const doc = document.createElement('div');
    let type = "img"
    let  currentPage = 1;
    let totalPages = 1
    const confirm = (item)=>{
        editor.getSelected().set({"src" : DOMAIN+item.path });
        editor.getSelected().setStyle({"width" : "100%" });
        runCommands(CALLBACK_MODAL_SELECT_IMAGE,{...item})
    }
