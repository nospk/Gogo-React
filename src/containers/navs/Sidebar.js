...
import {countByStatus} from '../../redux/order/actions'

import {deliveryCount} from "../../services/delivery/DeliveryService";
import {deliveryOrderCount} from "../../services/delivery/OrderDeliveryService";
import {deliveryCurrencyCount} from "../../services/delivery/CurrencyDeliveryService";
...
class Sidebar extends Component {
  ...
  componentDidMount() {
    ...
    this.handleProps();
    this.setSelectedLiActive(this.setHasSubItemStatus);
    this.props.countByStatus("processing")
    deliveryCount().then((results)=>{
      this.setState({
        delivery:results.count
      })
    });
    deliveryOrderCount().then((results)=>{
      this.setState({
        deliveryOrder:results.count
      })
    });
    deliveryCurrencyCount().then((results)=>{
      this.setState({
        deliveryCurrency:results.count
      })
    });
  }
  ...
  renderBadge(type){
    switch (type){
      case "deliveryall":return(
          <Badge color="danger" className="">{this.state.delivery+this.state.deliveryOrder+this.state.deliveryCurrency}</Badge>
      )
      case "delivery":return(
          <Badge color="danger" className="">{this.state.delivery}</Badge>
      )
      case "deliveryorder":return(
          <Badge color="danger" className="">{this.state.deliveryOrder}</Badge>
      )
      case "deliverycurrency":return(
          <Badge color="danger" className="">{this.state.deliveryCurrency}</Badge>
      )
      default: return "";
    }
  }
...
const mapStateToProps = ({ menu,orderRedux }) => {
  const {
    containerClassnames,
    subHiddenBreakpoint,
