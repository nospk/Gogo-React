...
class SettingBanner extends Component{
    ...
    componentDidMount() {
        SettingService.findAllBannerTOP().then((results)=>{
            this.setState({
    ...
    submit(){
        SettingService.updateBannerTOP(this.state.banner).then((results)=>{
    ...
    remove(id){
        SettingService.deleteBannerTOP(id).then((results)=>{
            window.location.reload();
        })
    }
    renderItem(){
        return this.state.banners.map((item)=>{
            ...
            return(
                <Col className="position-relative" key={id} xs={6} sm={6} md={3} xl={3}>
                    <img onClick={()=>this.edit(item)} className="w-100" src={thumbnail?DOMAIN+thumbnail:imgNotFound} />
            ...
            <div>
                <Card>
                    <CardHeader className="pt-3 d-flex justify-content-between align-items-center">
                        <IntlMessages id="menu.setting.banner1"/>
                        <div onClick={() => {
                            this.add()
