...
import {Card, CardBody, Badge, CustomInput, Col, Row, Button, Collapse} from "reactstrap";
...
const DeliveryOrderItem = ({item, changeStatus, dispatch}) => {
    ...
    const update = () => {
        ...
        updateOrder(dataTemp).then((results)=>{
            setEnableEdit(!enableEdit);
        })
    ...
    const handlerOption = (e) => {
        addStatus({id:item.id,status:e.target.value}).then((results)=>{
    ...
    const renderLinks = ()=>{
        const links = dataTemp.link?dataTemp.link.split(","):[];
        return(
            <div className="d-flex flex-column mt-3" style={{gap:10}}>
                <p className="font-weight-bold">Danh sách link sản phẩm</p>
                {
                ...
                <div className="min-width-zero">
                    <CardBody>
                        <Row>
                            ...
                            <Col xs={3} sm={3} xl={3} md={3} className="d-flex flex-column align-items-end">
                                <div>
                                    {renderOptions("")}
                                </div>
                                <div>
                                    <div className="d-flex flex-column mt-2">
                                        ...
                                        <Button className="mt-2" outline color={"secondary"}
                                                onClick={() => setCollapse(!collapse)}>Chi Tiết</Button>
                                    </div>
                                </div>
                            </Col>
                        </Row>
...
export default React.memo(DeliveryOrderItem);
